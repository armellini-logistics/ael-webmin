...
sub get_extended_sysinfo
{
    ...
    if ($info_ref) {
        ...
        # Remote Webmin Servers Accordion
        if (&webmin_user_is_admin() &&
            $theme_config{'settings_sysinfo_hidden_panels_user'} !~ /\'remote_servers\'/ &&
            &foreign_check("servers"))
        {
            &foreign_require("servers", "servers-lib.pl");
            my @servers = &servers::list_servers();
            if (@servers) {
                my $data = '<div class="table-responsive"><table class="table table-striped table-hover">';
                $data .= '<thead><tr>';
                $data .= '<th>Server Name</th>';
                $data .= '<th>IP Address / Hostname</th>';
                $data .= '</tr></thead><tbody>';
                foreach my $s (@servers) {
                    my $name = $s->{'desc'} ? $s->{'desc'} : $s->{'host'};
                    my $proto = $s->{'ssl'} ? 'https' : 'http';
                    my $port = $s->{'port'} ? ":$s->{'port'}" : "";
                    my $url = "$proto://$s->{'host'}$port/";
                    $data .= '<tr>';
                    $data .= '<td><a href="' . &html_escape($url) . '" target="_blank"><strong>' . &html_escape($name) . '</strong> <i class="fa fa-external-link" style="font-size: 10px; margin-left: 4px;"></i></a></td>';
                    $data .= '<td>' . &html_escape($s->{'host'}) . '</td>';
                    $data .= '</tr>';
                }
                $data .= '</tbody></table></div>';
                
                $returned_sysinfo .= print_panel(
                    1,
                    'remote_servers',
                    "Connected Remote Servers<span class=\"pull-right on-hover\"><i class=\"fa fa-fw fa-times-thin\" " .
                    get_button_tooltip(theme_text('theme_xhred_tooltip_dashboard_panels_disable', 'Connected Remote Servers'), undef, 'auto right', undef, undef, '#system-status .panel-body') .
                    "></i></span>",
                    $data,
                    1,
                    'A',
...
sub theme_list_sysinfo_accordion_candidates
{
    ...
    if (&webmin_user_is_admin() && !$seen{'remote_servers'}++ && &foreign_check("servers")) {
        push(@accordions, ['remote_servers', 'Connected Remote Servers', 'A']);
    }
    my %panels_order = theme_sysinfo_panels_order();
    if (%panels_order) {
        foreach my $accordion (@accordions) {
            next if ($accordion->[0] eq 'live_stats' || $accordion->[0] eq 'remote_servers');
